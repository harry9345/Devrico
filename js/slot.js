const Slot = function (canvas) {
  this.canvas = canvas;
  let auto = false;
  let reels = [...range(1, 3)].map(
    (i) => new Reel(canvas, conf.reel.xOffsets[i - 1])
  );
  ...
  this.drawStopPoints = function () {
    this.canvas.strokeStyle = "#00a1ff";
  ...
  let check = function (reels) {
    if (
      reels[0][0].stop !== reels[1][0].stop ||
      reels[1][0].stop !== reels[2][0].stop
    ) {
      //no win
      return;
    }
